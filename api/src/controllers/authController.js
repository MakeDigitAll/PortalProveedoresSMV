...
const auth = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      const isPasswordMatch = await verifyPassword(password, user.user.password);

      if (isPasswordMatch) {

        const permission = await pool.query('SELECT * FROM "Permissions" WHERE "userId" = $1', [user.user.id]);

        const accessToken = jwt.sign({ username: user.user.userName, roles: permission.rows[0].permission}, token.secretKey, { expiresIn: '1h' });

        const refreshToken = jwt.sign({ username: user.user.userName }, token.refreshTokenSecretKey, { expiresIn: '30d' });

        await saveRefreshToken(refreshToken, user.user.id);

        res.cookie('makeID', refreshToken, { httpOnly: true, sameSite: 'none', maxAge: 30 * 24 * 60 * 60 * 1000, secure: true });

        res.json({ accessToken: accessToken, 
                   roles: permission.rows[0].permission, 
                   isVerified: user.user.isVerified, 
                   referenceCode: user.refCode, 
...
const getAuth = async (username) => {
  try {
    ...
    const refCode = await pool.query('SELECT "referenceCode" FROM "providersProfile" WHERE "providerId" = $1', [response.rows[0].id]);
    if(!refCode.rows[0]){
    ...
    const resp = {
      user: response.rows[0],
      refCode: refCode.rows[0].referenceCode
    };
    return resp;
  } catch (error) {
    throw new Error('Usuario no encontrado');
...
const saveRefreshToken = async (refreshToken, userId) => {
  try {
    await pool.query('UPDATE "userAuth" SET "refreshToken" = $1 WHERE "id" = $2', [refreshToken, userId]);
  } catch (error) {
    throw new Error('Error al guardar el refreshToken');
  }
};
